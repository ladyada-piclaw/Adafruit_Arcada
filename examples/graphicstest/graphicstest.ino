...
void setup(void) {
  ...
  // a single pixel
  arcada.display->drawPixel(arcada.display->width() / 2,
                            arcada.display->height() / 2, ARCADA_GREEN);
  delay(500);
...
void testlines(uint16_t color) {
  ...
  arcada.display->fillScreen(ARCADA_BLACK);
  for (int16_t x = 0; x < arcada.display->width(); x += 6) {
    arcada.display->drawLine(arcada.display->width() - 1, 0, x,
                             arcada.display->height() - 1, color);
    delay(0);
  }
  for (int16_t y = 0; y < arcada.display->height(); y += 6) {
  ...
  for (int16_t y = 0; y < arcada.display->height(); y += 6) {
    arcada.display->drawLine(0, arcada.display->height() - 1,
                             arcada.display->width() - 1, y, color);
    delay(0);
  }

  arcada.display->fillScreen(ARCADA_BLACK);
  for (int16_t x = 0; x < arcada.display->width(); x += 6) {
    arcada.display->drawLine(arcada.display->width() - 1,
                             arcada.display->height() - 1, x, 0, color);
    delay(0);
  }
  for (int16_t y = 0; y < arcada.display->height(); y += 6) {
    arcada.display->drawLine(arcada.display->width() - 1,
                             arcada.display->height() - 1, 0, y, color);
    delay(0);
  }
}
...
void testdrawrects(uint16_t color) {
  arcada.display->fillScreen(ARCADA_BLACK);
  for (int16_t x = 0; x < arcada.display->width(); x += 6) {
    arcada.display->drawRect(arcada.display->width() / 2 - x / 2,
                             arcada.display->height() / 2 - x / 2, x, x, color);
  }
}
...
void testfillrects(uint16_t color1, uint16_t color2) {
  ...
  for (int16_t x = arcada.display->width() - 1; x > 6; x -= 6) {
    arcada.display->fillRect(arcada.display->width() / 2 - x / 2,
                             arcada.display->height() / 2 - x / 2, x, x,
                             color1);
    arcada.display->drawRect(arcada.display->width() / 2 - x / 2,
                             arcada.display->height() / 2 - x / 2, x, x,
                             color2);
...
void testdrawcircles(uint8_t radius, uint16_t color) {
  for (int16_t x = 0; x < arcada.display->width() + radius; x += radius * 2) {
    for (int16_t y = 0; y < arcada.display->height() + radius;
         y += radius * 2) {
