...
void loop() {
  ...
  int32_t startReal =
              (int64_t)((centerReal - rangeReal * 0.5) * (float)(1 << bits)),
          startImag =
              (int64_t)((centerImag + rangeImag * 0.5) * (float)(1 << bits)),
          incReal =
              (int64_t)((rangeReal / (float)pixelWidth) * (float)(1 << bits)),
          incImag =
              (int64_t)((rangeImag / (float)pixelHeight) * (float)(1 << bits));
  ...
  for (int y = 0; y < pixelHeight; y++) {
    ...
    for (int x = 0; x < pixelWidth; x++) {
      ...
      // tft.drawPixel(x, y, (n * 29)<<8 | (n * 67)); // takes 500ms with
