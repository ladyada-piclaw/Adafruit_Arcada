...
uint16_t samplerates[NUM_SAMPLERATES] = {50,   100,  250,  500,  1000, 1500,
...
void setup(void) {
  ...
  for (int h = 0; h < 240; h++) {
    for (int w = 0; w < DIGIKEY_LOGO_HEIGHT; w++) {
      buffer[h * DIGIKEY_LOGO_HEIGHT + w] =
          digikey_logo[h * DIGIKEY_LOGO_HEIGHT + w];
    }
  }
...
void loop() {
  ...
  do {
    ...
    if ((buttons & ARCADA_BUTTONMASK_RIGHT) &&
        (justPressed & ARCADA_BUTTONMASK_LEFT)) {
      rate_mode = true;
      uint8_t i;
    ...
    if ((buttons & ARCADA_BUTTONMASK_LEFT) &&
        (justPressed & ARCADA_BUTTONMASK_RIGHT)) {
      rate_mode = true;
      uint8_t i;
      for (i = 0; i < NUM_SAMPLERATES; i++) {
        if (DELAY_PER_SAMPLE == samplerates[i]) {
...
void printTicks(GFXcanvas16 *_canvas, uint8_t ticks, float minY, float maxY) {
  ...
  for (int t = 0; t < ticks; t++) {
    ...
    uint16_t y = map(t, 0, ticks - 1, ARCADA_TFT_HEIGHT - PLOT_BOTTOMBUFFER - 4,
                     PLOT_TOPBUFFER);
