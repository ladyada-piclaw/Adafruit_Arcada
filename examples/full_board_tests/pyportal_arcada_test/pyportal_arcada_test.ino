...
uint8_t esp32_md5[] = {0x5E, 0x04, 0xC0, 0x5C, 0xD7, 0x71, 0xe6, 0x7F,
                       0x76, 0xA3, 0xD9, 0xe2, 0x71, 0x5a, 0x59, 0xCE};
...
void setup() {
  ...
  arcada.display->print("QSPI Flash JEDEC 0x");
  arcada.display->println(Arcada_QSPI_Flash.getJEDECID(), HEX);
  ...
  if (foundFS == ARCADA_FILESYS_NONE) {
    ...
    if (foundFS == ARCADA_FILESYS_SD)
      arcada.display->print("SD: ");
    if (foundFS == ARCADA_FILESYS_QSPI)
      arcada.display->print("QSPI: ");
    if (foundFS == ARCADA_FILESYS_SD_AND_QSPI)
      arcada.display->print("SD & QSPI: ");
  ...
  coin.initButton(arcada.display, 160, 200, 100, 50, ARCADA_WHITE,
                  ARCADA_YELLOW, ARCADA_BLACK, (char *)"Sound", 2);
  coin.drawButton();
...
void loop() {
  ...
  arcada.display->print("Light sensor: ");
  arcada.display->println(light);
  ...
  arcada.display->print("Temp sensor: ");
  arcada.display->println(temp, 2);
  ...
  Serial.print(d4, 1);
  Serial.println();
  arcada.display->print("D3: ");
  arcada.display->println(d3, 1);
  arcada.display->print("D4: ");
  arcada.display->println(d4, 1);
  ...
  if (p.z > 100) {
    ...
    arcada.display->print(p.x);
    arcada.display->print(", ");
    arcada.display->print(p.y);
...
boolean write_esp32(const char *filename, uint8_t *md5) {
  ...
  for (uint32_t i = 0; i < firmsize; i += MAX_PAYLOAD_SIZE) {
    ...
    Serial.print("Packet #");
    Serial.print(i / MAX_PAYLOAD_SIZE);
    Serial.print(": ");
