...
void setup(void) {
  ...
  arcada.accel->enableInterrupts(true, true, true, true,
                                 true); // enable single/double and xyz activity
...
void loop() {
  ...
  if (clickstat) {
    Serial.print("Click = 0x");
    Serial.println(clickstat, HEX);
  }
  uint8_t motionstat = arcada.accel->getMotionInterruptStatus();
  if (motionstat) {
    Serial.print("Motion IRQ = 0x");
    Serial.println(motionstat, HEX);
    if (motionstat & (1 << 5)) {
  ...
  arcada.display->println(arcada.readLightSensor());
  ...
  Serial.print("D2/A8 Sensor: ");
  Serial.print(vsense);
  Serial.println("V");
  arcada.display->print("D2/A8 Sensor: ");
  arcada.display->print(vsense);
  arcada.display->println("V");
  vsense = analogRead(A9) * 3.3 / 1024;
  Serial.print("D3/A9 Sensor: ");
  Serial.print(vsense);
  Serial.println("V");
  arcada.display->print("D3/A9 Sensor: ");
  arcada.display->print(vsense);
  arcada.display->println("V");
