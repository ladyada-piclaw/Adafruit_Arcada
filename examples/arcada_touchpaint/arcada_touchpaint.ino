  ...
  if (p.x < BOXSIZE) {
    ...
    if (oldcolor != currentcolor) {
      ...
      if (oldcolor == ARCADA_GREEN)
        arcada.display->fillRect(0, BOXSIZE * 2, BOXSIZE, BOXSIZE,
                                 ARCADA_GREEN);
      ...
      if (oldcolor == ARCADA_MAGENTA)
        arcada.display->fillRect(0, BOXSIZE * 5, BOXSIZE, BOXSIZE,
                                 ARCADA_MAGENTA);
    }
  }
  if (((p.x - PENRADIUS) > BOXSIZE) &&
      ((p.x + PENRADIUS) < arcada.display->width())) {
