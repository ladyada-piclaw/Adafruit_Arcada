...
void setup() {
  ...
  if (!arcada.createFrameBuffer(arcada.display->width(),
                                arcada.display->height())) {
  ...
  pixeldust =
      new Adafruit_PixelDust(play_width, play_height, N_GRAINS, 1, 128, false);
  ...
  for (int i = 0; i < N_COLORS; i++) {
    colors[i] =
        __builtin_bswap16(colors[i]); // we swap the colors here to speed up DMA
  }
#endif
}
...
void loop() {
  ...
  for (int yl = 0; yl < LOGO_HEIGHT; yl++) {
    for (int xl = 0; xl < LOGO_WIDTH; xl++) {
      uint16_t c = __builtin_bswap16(arcada.display->color565(
          logo_gray[yl][xl], logo_gray[yl][xl], logo_gray[yl][xl]));
